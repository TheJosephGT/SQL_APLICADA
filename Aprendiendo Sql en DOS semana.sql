create database cursoSqlAplicada
...
--2 - Crear una tabla (create table - sp_tables - sp_columns - drop table)
 ...
 exec sp_tables @table_owner='dbo'; --Ver tablas creadas
...
insert into agenda(apellido, nombre, telefono) values ('Moreno', 'Julian','8097772222')
...
insert into libros(titulo, autor, editorial) values ('La mariposa', 'Jualian', 'Planeta')
...
create table empleados(
 ...
 select nombre, sexo from empleados where nombre = 'Juan Perez'
...
update agenda set telefono=4458787
...
ç if object_id('visitantes') is not null
...
-- 37 - Agrupar registros (group by)
 ...
 select ciudad, count(*) as 'cantidad con el mail'
  from visitantes
